...
if version < bindingsVersion then
    if version < bindingsVersionMinSupported then
        ...
        -- Version 6: apply the user's OpenMW VR Input Settings choice rather than
        -- changing the shipped default binding table. Match the conflict behavior
        -- of the settings UI by clearing Auto Move from the same control.
        local profile = '/interaction_profiles/oculus/touch_controller'
        local controller = '/user/hand/right'
        local click = '/user/hand/right/input/thumbstick/click'
        local sneak = bindingSection:getCopy('ToggleSneak_Alias_VR')
        if sneak and sneak.paths then
            sneak.paths[profile] = sneak.paths[profile] or {}
            sneak.paths[profile][controller] = click
            bindingSection:set('ToggleSneak_Alias_VR', sneak)
        end
        local autoMove = bindingSection:getCopy('AutoMove_Alias_VR')
        if autoMove and autoMove.paths and autoMove.paths[profile] then
            autoMove.paths[profile][controller] = nil
            bindingSection:set('AutoMove_Alias_VR', autoMove)
        end
    end
